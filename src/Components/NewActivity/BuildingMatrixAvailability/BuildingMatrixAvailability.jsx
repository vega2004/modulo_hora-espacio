...
const BuildingAvailabilityMatrix = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const aulasUnicas = [...new Set(data.map(c => c.aula))];
    ...
    const renderCelda = (aula, hora) => {
        ...
        return <td className="celda-edificio-libre" key={`${aula}-${hora}`}></td>;
    };
            ...
            <table className="matriz-edificio-disponibilidad">
                <thead>
                    <tr>
                        <th>Hora</th>
                        {aulas.map(aula => {
                            const claseAula = clases.find(c => c.aula === aula);
                            const capacidad = claseAula?.capacidad ?? '—';

                            return (
                                <th key={aula}>
                                    {aula}
                                    <br />
                                    <small>Cap: {capacidad} alumnos</small>
                                </th>
                            );
                        })}
                    </tr>
                ...
                <tbody>
                    {HORAS.map((hora, i) => (
                        <tr key={hora}>
                            <td className="hora-columna">{`${hora} - ${HORAS[i + 1] || '21:00'}`}</td>
